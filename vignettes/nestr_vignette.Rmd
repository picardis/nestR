...
The central function for the first part of the `nestR` workflow is `find_nests`. This function identifies potential nests based on patterns of location revisitation. It takes as input the GPS data and returns any revisited locations that match a set of user-defined parameter values. Parameters can be conceptually organized in three groups:
...
The output of `find_nests` is a `data.frame` including any revisited locations that match the criteria specified in the function arguments, along with their  revisitation stats. For example:
...
In the case of the Jacksonville stork, the nesting attempt at the true nest was estimated to start on March 3rd 2009 and end on June 22nd 2009. The second, third, and fourth location were all visited in a time range that does not  overlap with that interval. The first instance of a location that was visited  simultaneously to the true nest is the fifth location in the set. Since wood  storks are able to do repeated nesting attempts, we can choose to be  conservative and select the fifth location as a non-nest to compare the nest to, rather than the second. 
...
Good news! The function identified the known nest correctly for burst  1134370-2013. However, it also identified a second location for the same burst. This looks like a second, probably failed, nesting attempt. We know that after  leaving Rookery Bay, this wood stork migrated north to the Atlantic coast where its summer range is, and we cannot exclude that it attempted to nest a second  time there. 

No nest was identified for burst 414590-2009 with this set of parameters. Let's  see what happens with the other set of parameters:
...
This time the correct nest for 414590-2009 was returned as well as the two nests we found with the previous set of parameters for 1134370-2013. Merging both results together leaves us with a `data.frame` of nesting attempts that will be the starting point for the second part of the workflow, where we will estimate reproductive outcome. 

```{r merge outputs, eval = FALSE}
