...
The second element of the list that `find_nests` returns as output is a `data.frame` named `visits`: this is essentially an updated version of the GPS data provided in input, with an additional column that flags any records taken at a nest with the location ID of that nest. Therefore, this `data.frame` contains the revisitation history of all nests listed in `nests`. This is what it looks like:
...
head(wost_output_1$nests %>% filter(burst == "721290-2010"), 10)
...
The model specification includes two processes: the survival process, which is not directly observable, and the observation process, which is the signal we get in the revisitation data. Specifically, the latent nest survival variable is modelled at the daily scale as a function of survival status at the previous time-step and survival probability:
...
``` {r run JAGS, eval = FALSE, echo = FALSE}
...
``` {r wost_outcomes structure, eval = FALSE, echo = FALSE}
str(wost_outcomes)

```


``` {r custom summary plots, eval = FALSE, echo = FALSE}
...
``` {r coda diagnostics, eval = FALSE, echo = FALSE}

# Note that the JAGS estimation returns a list of mcarray objects. To use
  # the coda diagnostic functions, you must us as.mcmc.list()
